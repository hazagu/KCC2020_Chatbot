...
namespace Dispatch_testbot
{
    public class DispatchBot : ActivityHandler
    {
        ...
        protected override async Task OnMessageActivityAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            ...
            var recognizerResult = await _botServices.Dispatch.RecognizeAsync(turnContext, cancellationToken); // 이게 문제

            const string WelcomeText2 = "안녕하세요! MannerBot 입니다. 현재 3 수정중 입니다.";
        ...
        {
            const string WelcomeText = "안녕하세요! MannerBot 입니다. 현재 1 수정중 입니다.";
        ...
        {
            const string WelcomeText = "안녕하세요! MannerBot 입니다. 현재 4 수정중 입니다.";
            await turnContext.SendActivityAsync(MessageFactory.Text($"{WelcomeText}"), cancellationToken);

            switch (intent)
            {
                case "l_manner":
                    await ProcessLuisMannerAsync(turnContext, recognizerResult.Properties["luisResult"] as LuisResult, cancellationToken);
                    break;
                case "manner_qna":
                    await ProcessSampleQnAAsync(turnContext, cancellationToken);
                    break;
                default:
                    _logger.LogInformation($"1Dispatch unrecognized intent: {intent}.");
                    await turnContext.SendActivityAsync(MessageFactory.Text($"Dispatch unrecognized intent: {intent}."), cancellationToken);
                    break;
            }
        }

        private async Task ProcessLuisMannerAsync(ITurnContext<IMessageActivity> turnContext, LuisResult luisResult, CancellationToken cancellationToken)
        {
